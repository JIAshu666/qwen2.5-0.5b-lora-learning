...
**第一次在魔搭社区 CPU 环境中完成 LoRA 微调的全流程记录**
...
这是我第一次在**魔搭社区 CPU 环境**中完成 LoRA 微调的全流程记录。项目使用 `ms-swift 4.5.3` 框架，对 `Qwen/Qwen2.5-0.5B-Instruct` 进行了轻量级 LoRA 微调，并保存了训练脚本、推理脚本、数据集样例和最终权重，便于复现和回顾。
...
- [ ] 在 GPU 环境中使用更大的模型（如 `Qwen2.5-3B` 或 `7B`）进行微调或者继续使用`Qwen2.5-0.5B`，对比GPU相对CPU的训练速度变化。
- [ ] 尝试更大的 `lora_rank` 和更多训练轮数，观察效果提升
